...
function replaceStringsWithRequires(string, useAbsoluteUrls) {
  return string.replace(stringRegex, function (match, quote, url) {
    if (!useAbsoluteUrls && url.charAt(0) !== ".") {
      url = "./" + url;
    }
    return "require('" + url + "')";
  });
...
module.exports = function(source, sourcemap) {
  ...
  var useAbsoluteUrls = config.useAbsoluteUrls || false;
  ...
  var newSource = source.replace(templateUrlRegex, function (match, url) {
                 ...
                 return templateProperty + ":" + replaceStringsWithRequires(url, useAbsoluteUrls);
               })
               .replace(stylesRegex, function (match, urls) {
                 ...
                 // if `keepUrl` query parameter is set to true.
                 return styleProperty + ":" + replaceStringsWithRequires(urls, useAbsoluteUrls);
               });
